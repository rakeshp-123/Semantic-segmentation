...
def layers(vgg_layer3_out, vgg_layer4_out, vgg_layer7_out, num_classes):
    ...
    # TODO: Implement function
    sdtDev_weight = 0.01
    regularize_weight = 1e-3
    #layer 7 1x1 convolution
    conv_1x1_layer_7 = tf.layers.conv2d(vgg_layer7_out, num_classes, 1, padding = 'same', 
                     kernel_initializer = tf.random_normal_initializer(stddev=sdtDev_weight),
                     kernel_regularizer= tf.contrib.layers.l2_regularizer(regularize_weight),
                     name='conv_1x1_layer_7')
    #upsample by 2
    upsample1 = tf.layers.conv2d_transpose(conv_1x1_layer_7,
                                           num_classes,
                                           4,
                                           strides= (2, 2),
                                           padding= 'same',
    ...
    #upsample by 2
    skipNode1 = tf.add(upsample1, conv_1x1_layer_4, name = 'skip_1')
    
    upsample2 = tf.layers.conv2d_transpose(skipNode1,
                                           num_classes,
                                           4,
                                           strides= (2, 2),
                                           padding= 'same',
    ...
    skipNode2 = tf.add(upsample2, conv_1x1_layer_3, name = 'skip_2')
    #upsample by 8
    upsample3 = tf.layers.conv2d_transpose(skipNode2,
                                           num_classes,
                                           16,
                                           strides= (8, 8),
                                           padding= 'same',
...
def optimize(nn_last_layer, correct_label, learning_rate, num_classes):
    ...
    """
    # TODO: Implement function
    #reshape the lasst layer and correct label as matrix with row having pixel andclolumn having classes
    logits = tf.reshape(nn_last_layer, (-1, num_classes))
    correct_label = tf.reshape(correct_label,(-1, num_classes))
    #define loss function
    cross_entropy_loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits = logits, labels = correct_label))
            
    # Optimzer with learing_rate
...
def train_nn(sess, epochs, batch_size, get_batches_fn, train_op, cross_entropy_loss, input_image,
    ...
    for epoch in range(epochs):
        ...
        print('loss per epoch: {}'.format(accumulated_loss))
        print()
...
def run():
    ...
    with tf.Session() as sess:
        ...
        ip_image, keep_prob, out_layer3, out_layer4, out_layer7 = load_vgg(sess, vgg_path)
        ...
        train_nn(sess, epochs, batch_size, get_batches_fn, train_op, cross_entropy_loss, ip_image,
                 gt_label, keep_prob, learning_rate)
                                        
        # TODO: Save inference data using helper.save_inference_samples
        helper.save_inference_samples(runs_dir, data_dir, sess, image_shape, logits, keep_prob, ip_image)
